...
{if $pageType == 'book' }
              <button class='button button2' style='/*vertical-align:middle*/' onclick='toggleBookOnLibrary({$bookID});'><i id='library-status-icon' class='logo {if $onLibrary }"remove"{else}"addition"{/if}'></i> <p> {if $onLibrary }"Remove Book From Library"{else}"Add Book to Library"{/if}</p></button>
{/if}
...
{foreach $userBooks as $row}
                <a href="book-list/?title={$row.title}&options=edit" id="book{$row@index}-edit-button" class='button button2 purple' style='/*vertical-align:middle;display:none*/'><i class='logo pencil'></i><p>{$row.title}</p></a>
{/foreach}
{/if}
