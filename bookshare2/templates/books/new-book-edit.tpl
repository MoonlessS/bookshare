<form method='POST' action='start-new-book/'>
        <div class='book-title-container'>
            <div id='title' width=40%>
              <input value='$title' type='text' class='invisible' name='title' placeholder='Your new book title' required style='min-height: 28px;' pattern='[a-zA-Z0-9 _-]{literal}{1,60}{/literal}' title='Title should have at least 1 and a maximum of 60 characters between letters, numbers, hyphen or underscore!'></div>
        ...
        <div>
          <div class='float border' style='width:250px;max-height:350px;margin-left: 20px;'>
            ...
            <div class='green title' style='margin-right:10px;top:-300px;'>New book Cover URL:<input type='text' class='invisible' name='url' placeholder='http://img.pt/img.png' pattern='(file|gopher|news|nntp|telnet|https?|ftps?|sftp)://([a-z0-9-]+\.)+[a-z0-9]{literal}{0,1000}{/literal}.*' title='URL format not correct!\nhttp://example.com/image.png' value='{$url}'></div>
          </div>
